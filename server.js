...
// FIX 1: Replaced deepseek-v4-pro-0813 (coding model, causes RP runaway)
//         Replaced kimi-k3 (requires fixed temp/top_p, causes 400)
const MODEL_MAPPING = {
  'gpt-3.5-turbo': 'nvidia/nemotron-3-super-120b-a12b',     // was deepseek-v4-pro-0813
  'gpt-4':         'deepseek-ai/deepseek-v4-flash-0731',
  'gpt-4-turbo':   'moonshotai/kimi-k2.6',                   // was kimi-k3
...
// FIX 2: Removed duplicate /v1/models endpoint (Express was ignoring the second one)
app.get('/v1/models', (req, res) => {
  const models = Object.keys(MODEL_MAPPING).map(id => ({
    id, object: 'model', created: 1700000000, owned_by: 'nvidia-nim-proxy'
  }));
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    // FIX 3: Disable thinking mode for DeepSeek V4 — prevents very slow responses
    const isDeepSeekV4 = nimModel.includes('deepseek-v4-flash') || nimModel.includes('deepseek-v4-pro');
    if (isDeepSeekV4) {
    ...
    if (stream) {
      ...
      // FIX 4: Added writableEnded checks to prevent crash on already-closed streams
